...
export default function LoginPage({ setUser }) {
    ...
    const handleLogin = async (e) => {
        ...
        try {
            ...
            setUser(data.user); // ou o que seu backend retorna
            setIsLoading(false);

            // Redirecionar
            if (localStorage.getItem('pendingBook') && redirect === 'payment') {
                navigate('/payment');
            } else {
    ...
    const handleRegister = async (e) => {
        ...
        // Separar nome completo em firstName e lastName (exemplo simples)
        const [firstName, ...rest] = registerData.name.trim().split(' ');
        ...
        try {
            ...
            if (!response.ok) {
                const errorData = await response.json();
                alert(`Erro ao registrar: ${errorData.message || response.statusText}`);
                setIsLoading(false);
                return;
            }
